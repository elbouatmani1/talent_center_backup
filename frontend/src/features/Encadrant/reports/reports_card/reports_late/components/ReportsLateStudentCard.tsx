...
const ReportsLateStudentCard: FunctionComponent<ReportsLateStudentCardProps> = ({
  ...
  return (
    <article
      className={`${cardClassName} cursor-pointer transition-colors hover:border-[#fca5a5] hover:shadow-[0_2px_8px_rgba(16,24,40,0.06)]`}
      role="button"
      tabIndex={0}
    ...
    >
      ...
      <div className="flex min-w-0 flex-col gap-3">
        <div>
          <p className={REPORTS_LATE_FIELD_LABEL}>Last Report:</p>
        ...
        <div>
          <p className={REPORTS_LATE_FIELD_LABEL}>Next Report:</p>
          ...
          {student.isOverdue ? (
            ...
            <p className={REPORTS_LATE_FIELD_SUB}>Due: {student.nextReportDue}</p>
          )}
        </div>
      </div>
      ...
      <span
        className={`${REPORTS_LATE_STATUS_BADGE} ${statusStyle.badgeBg} ${statusStyle.badgeText}`}
      >
        {statusStyle.label}
      </span>
    </article>
