...
const ExemptedStudentsDetailTable: FunctionComponent = () => {
  ...
  const statusBadge = (
    <span className="inline-flex rounded-full bg-[#dbeafe] px-2.5 py-1 font-inter text-xs font-medium leading-4 text-[#1d4ed8]">
      Exempted
    </span>
  );
            ...
            <thead>
              <tr className={`h-10 ${borderRow}`}>
                {headings.map((h) => (
                  <th
                    key={h}
                    className={`align-middle px-2 py-2 pb-2 font-inter text-sm font-bold leading-5 text-[#717182] first:pl-0 last:pr-2 ${
