...
/** Colonne Restant — rouge #D93025, gras comme sur la maquette */
            ...
            <thead>
              <tr className={`h-10 ${borderRow}`}>
                {headings.map((h) => (
                  <th
                    key={h}
                    className={`align-middle px-2 py-2 pb-2 font-inter text-sm font-medium leading-5 text-[#717182] first:pl-0 last:pr-2 ${
              ...
              {rows.map((row: PartiallyPaidStudentRow) => (
                <tr key={row.id} className={borderRow}>
                  ...
                  <td className="h-[49px] px-2 align-middle md:px-4">
                    <span className="inline-flex rounded-full bg-[#FFEDD5] px-2.5 py-1 font-inter text-xs font-medium leading-4 text-[#9A3412]">
                      Partially Paid
                    </span>
                  </td>
