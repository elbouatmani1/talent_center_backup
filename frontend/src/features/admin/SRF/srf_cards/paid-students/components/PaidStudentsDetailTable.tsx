...
/** Aligné sur Figma : bordures `gray-200` = rgba(0,0,0,0.1) */
        ...
        <div className="flex h-9 w-full flex-row items-center gap-2 sm:gap-3">
          ...
          <div className="relative h-9 w-full shrink-0 sm:w-[180px]">
            <select
              ...
              className="box-border h-9 w-full cursor-pointer appearance-none rounded-lg border border-solid border-[rgba(0,0,0,0.1)] bg-white py-1 pl-3 pr-9 font-inter text-sm font-normal leading-5 text-[#0a0a0a] outline-none focus-visible:ring-2 focus-visible:ring-[#2b7fff]"
            >
            ...
            <thead>
              <tr className={`h-10 ${borderRow}`}>
                {headings.map((h) => (
                  <th
                    key={h}
                    className={`align-middle px-2 py-2 pb-2 font-inter text-sm font-medium leading-5 text-[#0a0a0a] first:pl-0 last:pr-2 ${
              ...
              {rows.map((row: PaidStudentDetailRow) => (
                <tr key={row.id} className={borderRow}>
                  ...
                  <td className="h-[49px] px-2 align-middle md:px-4">
                    <span className="inline-flex rounded-lg bg-honeydew px-2 py-0.5 font-inter text-xs font-medium leading-4 text-seagreen">
                      Paid
                    </span>
                  </td>
