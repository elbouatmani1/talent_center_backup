...
import type { DocumentsStatCard } from '../types';

const StatIcon: FunctionComponent<{ icon: DocumentsStatCard['icon'] }> = ({ icon }) => {
  if (icon === 'uploaded') return <Plus className="h-5 w-5" strokeWidth={2} />;
  ...
  return <FileText className="h-5 w-5" strokeWidth={2} />;
};
