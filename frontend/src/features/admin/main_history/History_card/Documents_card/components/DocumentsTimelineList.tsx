...
import type { DocumentsActionType, DocumentsHistoryRow } from '../types';

const actionBadgeClass: Record<DocumentsActionType, string> = {
  validate: 'bg-[#ede9fe] text-[#5b21b6]',
  create: 'bg-[#dcfce7] text-[#016630]',
  update: 'bg-[#dbeafe] text-[#193cb8]',
};

const rowCircleClass: Record<DocumentsActionType, string> = {
  validate: 'bg-[#7c3aed] text-white',
  create: 'bg-[#22c55e] text-white',
  update: 'bg-[#3b82f6] text-white',
};

const rowActionIcon = (type: DocumentsActionType) => {
  if (type === 'validate') return <Check className="h-4 w-4" strokeWidth={2.5} />;
...
const DocumentsTimelineList: FunctionComponent = () => {
  ...
  const [selectedRow, setSelectedRow] = useState<DocumentsHistoryRow | null>(null);

  const rows = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return documentsHistoryRows;
