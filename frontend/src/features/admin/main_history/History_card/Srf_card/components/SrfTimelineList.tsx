...
import type { SrfActionType, SrfHistoryRow } from '../types';

const actionBadgeClass: Record<SrfActionType, string> = {
  validate: 'bg-[#ede9fe] text-[#5b21b6]',
  update: 'bg-[#dbeafe] text-[#193cb8]',
  create: 'bg-[#dcfce7] text-[#016630]',
};

const rowCircleClass: Record<SrfActionType, string> = {
  validate: 'bg-[#ede9fe] text-[#7c3aed]',
  update: 'bg-[#dbeafe] text-[#2563eb]',
  create: 'bg-[#dcfce7] text-[#16a34a]',
};

const rowActionIcon = (type: SrfActionType) => {
  if (type === 'validate') return <CheckSquare className="h-4 w-4" strokeWidth={2} />;
...
const SrfTimelineList: FunctionComponent = () => {
  ...
  const [selectedRow, setSelectedRow] = useState<SrfHistoryRow | null>(null);

  const rows = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return srfHistoryRows;
    return srfHistoryRows.filter((row) =>
