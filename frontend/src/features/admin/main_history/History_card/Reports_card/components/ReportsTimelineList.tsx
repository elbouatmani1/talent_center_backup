...
import type { ReportsActionType, ReportsHistoryRow } from '../types';

const actionBadgeClass: Record<ReportsActionType, string> = {
  create: 'bg-[#dcfce7] text-[#016630]',
  validate: 'bg-[#ede9fe] text-[#5b21b6]',
  update: 'bg-[#dbeafe] text-[#193cb8]',
};

const rowCircleClass: Record<ReportsActionType, string> = {
  create: 'bg-[#dbeafe]',
  validate: 'bg-[#ede9fe]',
  update: 'bg-[#dbeafe]',
};

const rowActionIcon = (type: ReportsActionType) => {
  const cls = 'h-4 w-4 text-[#2563eb]';
  if (type === 'create') return <Plus className={cls} strokeWidth={2} />;
  if (type === 'validate') return <CheckSquare className={cls} strokeWidth={2} />;
  return <FilePenLine className={cls} strokeWidth={2} />;
};

const ReportsTimelineList: FunctionComponent = () => {
  const [query, setQuery] = useState('');
  const [selectedRow, setSelectedRow] = useState<ReportsHistoryRow | null>(null);

  const rows = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return reportsHistoryRows;
    return reportsHistoryRows.filter((row) =>
          ...
          {rows.map((row) => (
            ...
            >
              <div className="flex min-w-0 items-center gap-4">
                <span
                  className={`inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${rowCircleClass[row.actionType]}`}
                >
                  {rowActionIcon(row.actionType)}
                </span>
          ...
          <div className="w-full max-w-[680px] overflow-hidden rounded-[14px] border border-[rgba(0,0,0,0.1)] bg-white shadow-xl">
            ...
            <div className="space-y-5 px-5 py-4">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="space-y-1">
                  <p className="flex items-center gap-2 text-sm font-medium text-[#0a0a0a]">
                    <FileBarChart className="h-4 w-4 text-[#9ca3af]" />
                    Module
                  </p>
                ...
                <div className="space-y-1">
                  <p className="flex items-center gap-2 text-sm font-medium text-[#0a0a0a]">
                    <span className="text-[#9ca3af]">{rowActionIcon(selectedRow.actionType)}</span>
                    Action Type
                  </p>
                  <span
              ...
              <div>
                ...
                <p className="mt-1 text-sm leading-5 text-[#717182]">{selectedRow.details}</p>
              </div>
            </div>
