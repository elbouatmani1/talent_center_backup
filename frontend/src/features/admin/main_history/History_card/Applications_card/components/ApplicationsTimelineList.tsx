...
import type { ApplicationsActionType, ApplicationsHistoryRow } from '../types';

const actionBadgeClass: Record<ApplicationsActionType, string> = {
  create: 'bg-[#dcfce7] text-[#016630]',
  update: 'bg-[#dbeafe] text-[#193cb8]',
};

const rowActionIcon = (type: ApplicationsActionType) => {
  if (type === 'create') return <Plus className="h-4 w-4" strokeWidth={2} />;
  return <CheckSquare className="h-4 w-4" strokeWidth={2} />;
};

const ApplicationsTimelineList: FunctionComponent = () => {
  const [query, setQuery] = useState('');
  const [selectedRow, setSelectedRow] = useState<ApplicationsHistoryRow | null>(null);

  const rows = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return applicationsHistoryRows;
    return applicationsHistoryRows.filter((row) =>
