...
import type { ChatActionType, ChatHistoryRow } from '../types';

const actionBadgeClass: Record<ChatActionType, string> = {
  create: 'bg-[#dcfce7] text-[#016630]',
};

const ChatTimelineList: FunctionComponent = () => {
  const [query, setQuery] = useState('');
  const [selectedRow, setSelectedRow] = useState<ChatHistoryRow | null>(null);

  const rows = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return chatHistoryRows;
    return chatHistoryRows.filter((row) =>
          ...
          {rows.map((row) => (
            ...
            >
              <div className="flex min-w-0 items-center gap-4">
                <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#dbeafe] text-[#22c55e]">
