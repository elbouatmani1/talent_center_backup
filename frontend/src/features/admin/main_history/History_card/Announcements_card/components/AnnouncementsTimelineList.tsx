...
import type { AnnouncementsActionType, AnnouncementsHistoryRow } from '../types';

const actionBadgeClass: Record<AnnouncementsActionType, string> = {
  create: 'bg-[#dcfce7] text-[#016630]',
  update: 'bg-[#dbeafe] text-[#193cb8]',
  delete: 'bg-[#fee9eb] text-[#b4232d]',
};

const rowCircleClass: Record<AnnouncementsActionType, string> = {
  create: 'bg-[#dcfce7] text-[#15803d]',
  update: 'bg-[#dbeafe] text-[#1d4ed8]',
  delete: 'bg-[#fee2e2] text-[#b91c1c]',
};

const rowActionIcon = (type: AnnouncementsActionType) => {
...
const AnnouncementsTimelineList: FunctionComponent = () => {
  ...
  const [selectedRow, setSelectedRow] = useState<AnnouncementsHistoryRow | null>(null);

  const rows = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return announcementsHistoryRows;
    return announcementsHistoryRows.filter((row) =>
