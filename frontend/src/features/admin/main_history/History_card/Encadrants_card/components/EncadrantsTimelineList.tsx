...
import { CircleUserRound, Clock3, Eye, Funnel, Pencil, Plus, Search, ShieldCheck, Video, X } from 'lucide-react';
import { encadrantsRows } from '../data/encadrantsHistoryMock';
import type { EncadrantsHistoryActionType, EncadrantsHistoryRow } from '../types';

const actionBadgeClass: Record<EncadrantsHistoryActionType, string> = {
  update: 'bg-[#dbeafe] text-[#193cb8]',
  validate: 'bg-[#f3e8ff] text-[#6e11b0]',
  create: 'bg-[#dcfce7] text-[#016630]',
};

const actionIcon = (type: EncadrantsHistoryActionType) => {
  if (type === 'create') return <Plus className="h-4 w-4" strokeWidth={2} />;
  if (type === 'validate') return <ShieldCheck className="h-4 w-4" strokeWidth={2} />;
  return <Pencil className="h-4 w-4" strokeWidth={2} />;
};

const EncadrantsTimelineList: FunctionComponent = () => {
  const [query, setQuery] = useState('');
  const [selectedRow, setSelectedRow] = useState<EncadrantsHistoryRow | null>(null);

  const rows = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return encadrantsRows;
    return encadrantsRows.filter((row: EncadrantsHistoryRow) =>
