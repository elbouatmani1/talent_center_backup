...
import { BriefcaseBusiness, CircleUserRound, Clock3, Eye, Funnel, Pencil, Plus, Search, Trash2, X } from 'lucide-react';
import { internshipOffersRows } from '../data/internshipOffersHistoryMock';
import type { InternshipOffersActionType, InternshipOffersHistoryRow } from '../types';

const actionBadgeClass: Record<InternshipOffersActionType, string> = {
  create: 'bg-[#dcfce7] text-[#016630]',
  update: 'bg-[#dbeafe] text-[#193cb8]',
  delete: 'bg-[#fee9eb] text-[#b4232d]',
};

const actionIcon = (type: InternshipOffersActionType) => {
...
const InternshipOffersTimelineList: FunctionComponent = () => {
  ...
  const [selectedRow, setSelectedRow] = useState<InternshipOffersHistoryRow | null>(null);

  const rows = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return internshipOffersRows;
    return internshipOffersRows.filter((row: InternshipOffersHistoryRow) =>
