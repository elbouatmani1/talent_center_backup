...
import { CircleUserRound, Clock3, Eye, Funnel, Pencil, Plus, Search, ShieldCheck, X } from 'lucide-react';
import { adminsRows } from '../data/adminsHistoryMock';
import type { AdminHistoryActionType, AdminHistoryRow } from '../types';

const actionBadgeClass: Record<AdminHistoryActionType, string> = {
  update: 'bg-[#dbeafe] text-[#193cb8]',
  create: 'bg-[#dcfce7] text-[#016630]',
};

const actionIcon = (type: AdminHistoryActionType) => {
...
const AdminsTimelineList: FunctionComponent = () => {
  ...
  const [selectedRow, setSelectedRow] = useState<AdminHistoryRow | null>(null);

  const rows = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return adminsRows;
    return adminsRows.filter((row: AdminHistoryRow) =>
