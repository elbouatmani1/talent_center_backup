...
const StatIcon: FunctionComponent<{ icon: AdminStatCard['icon'] }> = ({ icon }) => {
  if (icon === 'update') return <Pencil className="h-5 w-5" strokeWidth={2} />;
  if (icon === 'create') return <Plus className="h-5 w-5" strokeWidth={2} />;
  if (icon === 'permission') return <ShieldCheck className="h-5 w-5" strokeWidth={2} />;
  return <UserRoundCog className="h-5 w-5" strokeWidth={2} />;
};
