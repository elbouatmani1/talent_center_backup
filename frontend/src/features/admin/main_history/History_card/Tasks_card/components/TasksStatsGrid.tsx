...
import { CheckSquare, ListChecks, Plus, SquarePen } from 'lucide-react';
import { tasksStats } from '../data/tasksHistoryMock';
import type { TasksStatCard } from '../types';

const StatIcon: FunctionComponent<{ icon: TasksStatCard['icon'] }> = ({ icon }) => {
  ...
  if (icon === 'updated') return <SquarePen className="h-5 w-5" strokeWidth={2} />;
  return <ListChecks className="h-5 w-5" strokeWidth={2} />;
};
