...
import type { TasksActionType, TasksHistoryRow } from '../types';

const actionBadgeClass: Record<TasksActionType, string> = {
  create: 'bg-[#dcfce7] text-[#016630]',
  update: 'bg-[#dbeafe] text-[#193cb8]',
};

const rowActionIcon = (type: TasksActionType) => {
  if (type === 'create')
    return <Plus className="h-4 w-4 text-[#22c55e]" strokeWidth={2} />;
  return <Pencil className="h-4 w-4 text-[#2563eb]" strokeWidth={2} />;
};

const TasksTimelineList: FunctionComponent = () => {
  const [query, setQuery] = useState('');
  const [selectedRow, setSelectedRow] = useState<TasksHistoryRow | null>(null);

  const rows = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return tasksHistoryRows;
    return tasksHistoryRows.filter((row) =>
          ...
          {rows.map((row) => (
            ...
            >
              <div className="flex min-w-0 items-center gap-4">
                <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#dbeafe]">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="space-y-1">
                  <p className="flex items-center gap-2 text-sm font-medium text-[#0a0a0a]">
                    <ListTodo className="h-4 w-4 text-[#9ca3af]" />
                    Module
                  </p>
