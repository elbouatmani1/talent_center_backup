...
import { CheckSquare, Plus, Video, X } from 'lucide-react';
import { meetingsStats } from '../data/meetingsHistoryMock';
import type { MeetingsStatCard } from '../types';

const StatIcon: FunctionComponent<{ icon: MeetingsStatCard['icon'] }> = ({ icon }) => {
  if (icon === 'scheduled') return <Plus className="h-5 w-5" strokeWidth={2} />;
  if (icon === 'completed') return <CheckSquare className="h-5 w-5" strokeWidth={2} />;
  if (icon === 'canceled') return <X className="h-5 w-5" strokeWidth={2} />;
  return <Video className="h-5 w-5" strokeWidth={2} />;
};
