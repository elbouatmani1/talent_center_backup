...
import type { MeetingsActionType, MeetingsHistoryRow } from '../types';

const actionBadgeClass: Record<MeetingsActionType, string> = {
  create: 'bg-[#dcfce7] text-[#016630]',
  update: 'bg-[#dbeafe] text-[#193cb8]',
  delete: 'bg-[#fee9eb] text-[#b4232d]',
};

const rowCircleClass: Record<MeetingsActionType, string> = {
  create: 'bg-[#dbeafe]',
  update: 'bg-[#dbeafe]',
  delete: 'bg-[#fee2e2]',
};

const rowActionIcon = (type: MeetingsActionType) => {
  if (type === 'create') return <Plus className="h-4 w-4 text-[#22c55e]" strokeWidth={2} />;
  if (type === 'delete') return <Trash2 className="h-4 w-4 text-[#b91c1c]" strokeWidth={2} />;
  return <CheckSquare className="h-4 w-4 text-[#2563eb]" strokeWidth={2} />;
};

const MeetingsTimelineList: FunctionComponent = () => {
  const [query, setQuery] = useState('');
  const [selectedRow, setSelectedRow] = useState<MeetingsHistoryRow | null>(null);

  const rows = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return meetingsHistoryRows;
    return meetingsHistoryRows.filter((row) =>
