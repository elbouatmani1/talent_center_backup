...
import { CheckCheck, Menu, MoreVertical, Paperclip, Search, Send, Tag } from 'lucide-react';
...
const AdminModuleChat: FunctionComponent<AdminModuleChatProps> = ({
  ...
  return (
    <AdminLayout mainFillHeight>
      <div className="font-inter -mx-5 -my-5 flex min-h-0 flex-1 flex-col md:-mx-6 md:-my-6">
        <div className="flex min-h-0 flex-1 overflow-hidden bg-white">
          <aside className="flex h-full min-h-0 w-full max-w-[100%] shrink-0 flex-col border-r border-solid border-neutral-200 bg-white sm:w-[clamp(260px,32vw,340px)]">
            <div className="flex min-h-[2.875rem] shrink-0 items-center gap-2 border-b border-solid border-neutral-100 px-2.5">
              <button
                type="button"
                aria-label="Menu"
                className="inline-flex size-9 shrink-0 items-center justify-center rounded-full text-[#404040] transition-colors hover:bg-neutral-100"
              >
              ...
              <div className="relative flex min-h-[2.25rem] min-w-0 flex-1 items-center">
                <input
                  ...
                  className="box-border inline-flex h-9 min-h-[2.25rem] w-full items-center rounded-full border-0 bg-[#f4f4f5] px-3.5 text-sm leading-normal text-[#0a0a0a] placeholder:text-[#717182] align-middle outline-none ring-2 ring-transparent focus:bg-[#ececee] focus:ring-[#2b7fff]/20"
                />
              </div>
            </div>
              ...
              {filteredConversations.map((c) => {
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-start gap-2.5 border-b border-solid border-neutral-100 px-2.5 py-2.5 text-left transition-colors ${
          ...
          <section className="relative flex h-full min-h-0 min-w-0 flex-1 flex-col bg-white">
            {selectedConv ? (
              <>
                <div className="relative z-[1] flex min-h-[2.875rem] shrink-0 items-center justify-between gap-3 border-b border-solid border-neutral-200 bg-white px-4">
                ...
                <div
                  ref={scrollRef}
                  className="relative z-[1] min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-4"
                >
                  {thread.map((msg) => (
                    <div key={msg.id}>
                      {msg.separatorBefore ? (
                ...
                <footer className="relative z-[1] box-border w-full max-w-full min-w-0 shrink-0 bg-transparent px-4 pb-[max(1rem,env(safe-area-inset-bottom,0px))] pt-2 md:px-5">
              ...
              <div className="relative z-[1] flex flex-1 items-center justify-center text-sm font-medium text-[#64748b]">
